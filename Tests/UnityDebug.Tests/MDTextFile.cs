...
namespace Mono.Debugging.Tests
{
    public class MDTextFile : IMDTextFile
    {
        ...
        static string ConvertFromEncoding(byte[] content, long nread, string fromEncoding)
        {
            ...
            {
                Console.WriteLine($"{fromEncoding}: failed to use. Exception: {e.Message}");
                return null;
            }
        }
...
#pragma warning restore 649
        ...
        public static bool WriteFile(string fileName, string content, string encoding, bool onlyIfChanged)
        {
            byte[] buf = Encoding.UTF8.GetBytes(content);
            return WriteFile(fileName, buf, encoding, onlyIfChanged);
        }
